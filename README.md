...
5. With all the necessary modules downloaded, we now have to start both of them. Enter the backend folder (cd backend) and run "python -m uvicorn app:app --host 127.0.0.1 --port 8000". This starts local port 8000 to run the Python server necessary for the database. Exit back to finalGradeCalculator then enter docs. Run "npm start". The website will open at port 3000.
